...
def create_exell_tab(db_path: str):
...
def get_line(line_numb:int, sql_db_path:str):
...
def ins_into_sql(work_id, rates):
    db = sqlite3.connect('db_dir/ege_works.db')
    curs = db.cursor()
...
def ins_into_exell(work_info, db_path):
    wb = load_workbook(db_path)
    ws = wb.active
    work_info = [excel_safe_value(el) for el in work_info]
    ws.append(work_info)
